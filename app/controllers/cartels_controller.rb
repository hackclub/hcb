...
class CartelsController < ApplicationController
  ...
  def promotions
    ...
    @is_argosy = @event.plan.is_a?(Cartel::Plan::Argosy2025)
  end
  ...
  def sub_organizations
    ...
    respond_to do |format|
      format.html do
        ...
        @all_cartels = [@event] + @event.descendants.order(:name).select(:name, :parent_id, :slug, :id).to_a
      end
  ...
  def filtered_sub_organizations(sub_organizations = @event.subevents)
    ...
    scoped_tag = Cartel::ScopedTag.find_by(name: params[:tag])
