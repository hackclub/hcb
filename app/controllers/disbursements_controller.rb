...
class DisbursementsController < ApplicationController
  ...
  def new
    ...
    if admin_signed_in?
      # Load all events once and reuse for both source and destination dropdowns.
      # No plan includes needed — plan attributes are not accessed in the admin rendering path.
      admin_events = Event.select(:name, :id, :demo_mode, :can_front_balance, :slug)
                          .reorder(Event::CUSTOM_SORT)
                          .to_enum.with_index
                          .sort_by { |e, i| [user_event_ids.index(e.id) || Float::INFINITY, i] }
                          .map(&:first)
      @allowed_source_events = admin_events
      @allowed_destination_events = admin_events
    else
      user_accesible_events = Event.where(id: current_user.events.pluck(:id) + current_user.events.collect(&:descendant_ids).flatten)

      @allowed_source_events = user_accesible_events.not_hidden.filter_demo_mode(false)
                                                    .to_enum.with_index
                                                    .sort_by { |e, i| [user_event_ids.index(e.id) || Float::INFINITY, i] }
                                                    .map(&:first)
      @allowed_destination_events = if @source_event&.plan&.unrestricted_disbursements_enabled?
