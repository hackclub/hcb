...
class DisbursementsController < ApplicationController
  ...
  def event_search
    ...
    base = if is_admin
           ...
           elsif !sending && params[:source_event_id].present?
             source_event = Event.find_by(id: params[:source_event_id])
             if source_event&.plan&.unrestricted_disbursements_enabled?
               manageable_ids = current_user.manageable_events.not_hidden.filter_demo_mode(false).pluck(:id)
               indexable_ids = Event.indexable.pluck(:id)
               Event.where(id: (manageable_ids + indexable_ids).uniq).order(Event::CUSTOM_SORT)
             else
