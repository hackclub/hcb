...
class DisbursementsController < ApplicationController
  ...
  def event_search
    ...
    base = if admin_signed_in?
              Event.select(:name, :id, :demo_mode, :slug, :can_front_balance, :financially_frozen)
                   .limit(10)
                   .reorder(Event::CUSTOM_SORT)
                   .includes(:plan)
            elsif !sending && @source_event&.plan&.unrestricted_disbursements_enabled?
              allowed_destination_event_ids = current_user.manageable_events.not_hidden.filter_demo_mode(false).select(:id) +
                                              Event.indexable.select(:id)
              Event.where(id: allowed_destination_event_ids)
                   .select(:name, :id, :demo_mode, :can_front_balance, :slug, :financially_frozen)
                   .includes(:plan)
            else
