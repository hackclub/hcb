...
class CardGrantsController < ApplicationController
  ...
  def create
    ...
    @card_grant = @event.card_grants.build(params.require(:card_grant).permit(:amount_cents, :email, :invite_message, :keyword_lock, :purpose, :one_time_use, :pre_authorization_required, :instructions).merge(sent_by: current_user, expiration_date: @event.card_grant_setting.expiration_preference_before_type_cast.days.from_now))
