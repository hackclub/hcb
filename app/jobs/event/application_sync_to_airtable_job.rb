...
class Event
  class ApplicationSyncToAirtableJob < ApplicationJob
    ...
    def perform(application)
      ...
      app["Address Line 2"] = application.address_line2
      ...
      app["Team Size"] = application.team_size
      app["Planning Duration"] = application.planning_duration
      app["Currently fiscally sponsored?"] = application.currently_fiscally_sponsored? ? "Yes" : "No"
      app["Previously applied?"] = application.currently_fiscally_sponsored? ? "Yes" : "No"
      app["Committed Amount"] = application.committed_amount
      app["(Adults) Annual Budget"] = application.annual_budget
      app["Funding Source"] = application.funding_source
