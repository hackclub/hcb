    ...
    class Nightly
      def run
        Reimbursement::ExpensePayout.pending.find_each(batch_size: 100) do |expense_payout|
          next if expense_payout.expense.report.user.default_payout_method&.details.is_a?(IlegalEntity::PayoutMethod::WiseTransfer)
        ...
        Reimbursement::PayoutHolding.pending.find_each(batch_size: 100) do |payout_holding|
          next if payout_holding.report.user.default_payout_method&.details.is_a?(IlegalEntity::PayoutMethod::WiseTransfer)
