...
json.embed do
  if hcb_code.present?
    if hcb_code.stripe_refund?
      ...
      json.description "#{activity.user&.possessive_name} #{Discord.link_to(hcb_code.event&.name, event_url(hcb_code.event))} card was declined for #{render_money(activity.trackable.amount_cents.abs)} at #{hcb_code.memo}"
