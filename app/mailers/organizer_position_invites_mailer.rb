...
class OrganizerPositionInvitesMailer < ApplicationMailer
  ...
  def notify
    if @invite.organizer_position_invite_request.present?
      @delivery_reason = "requested to join an organization on HCB."
      ...
      @delivery_reason = "applied for fiscal sponsorship with HCB."
      mail to: @invite.user.email_address_with_name, subject: "Thanks for applying for HCB 🚀"
    else
      @delivery_reason = "have been invited to join an organization on HCB."
