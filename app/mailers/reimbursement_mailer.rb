...
class ReimbursementMailer < ApplicationMailer
  def invitation
    ...
    return unless @report.user.reimbursement_notifications?
  ...
  def review_requested
    ...
    if @report.reviewer.present?
      return unless @report.reviewer.reimbursement_notifications?
      ...
      to = User.find(@report.event.organizer_positions.manager.pluck(:user_id)).excluding(@report.user).select(&:reimbursement_notifications?).map(&:email_address_with_name)
