...
class InvoiceMailer < ApplicationMailer
  def notify_organizers_sent
    ...
    @emails = @invoice.sponsor.event.users.where(organizer_positions: { role: :manager }).where(invoice_notifications: true).map(&:email_address_with_name)
  ...
  def notify_organizers_paid
    @invoice = params[:invoice]
    @emails = @invoice.sponsor.event.organizer_contact_emails { |users| users.where(invoice_notifications: true) }
    @emails = @emails.length > 10 && @invoice.creator.invoice_notifications? ? [@invoice.creator.email_address_with_name] : @emails
