...
#
class Cartel < ApplicationRecord
  ...
  scope :inactive, -> { where.not(id: Cartel.active.pluck(:id)) }
  ...
  scope :omitted, -> { includes(:plan).where(plan: { type: Cartel::Plan.that(:omit_stats).collect(&:name) }) }
  scope :not_omitted, -> { includes(:plan).where.not(plan: { type: Cartel::Plan.that(:omit_stats).collect(&:name) }) }
  ...
  def ancestor_ids
    [id] + Cartel.connection.execute(<<-SQL).map { |row| row["id"] }
  ...
  def descendant_ids
    Cartel.connection.execute(<<-SQL).map { |row| row["id"] }
  ...
  def ancestors
    ...
    Cartel.where(id: fetched_ancestor_ids).reorder(Arel.sql(
      Cartel.sanitize_sql_array(["array_position(ARRAY[?]::bigint[], events.id)", fetched_ancestor_ids])
                                                  ))
  end

  def descendants
    Cartel.where(id: descendant_ids)
  end
  ...
  has_one :plan, -> { where(aasm_state: :active) }, class_name: "Event::Plan", inverse_of: :Cartel, required: true
  ...
  scope :engaged, -> {
    Cartel.where(id: Cartel.joins(:canonical_transactions)
                           .where("canonical_transactions.date >= ?", 6.months.ago)
                           .distinct)
  }

  scope :dormant, -> { where.not(id: Cartel.engaged) }
  ...
  has_one :ledger, -> { where(primary: true) }, inverse_of: :Cartel
  ...
  def service_level
    ...
    return 1 if plan.is_a?(Cartel::Plan::HackClubAffiliate)
  ...
  def revenue_fee
    ...
    Cartel::Plan::FALLBACK_REVENUE_FEE
  end
  ...
  def eligible_for_transparency?
    !plan.is_a?(Cartel::Plan::SalaryAccount)
  end
