...
class OrganizerPosition < ApplicationRecord
  ...
  def self.role_at_least?(user, event, role)
    ...
    if role.to_s == "reader"
      return event.self_and_ancestor_organizer_positions.reader_access.where(user:).exists?
    end

    if role.to_s == "member"
      # Only check direct organizer positions, unless the user is a manager of an ancestor
      return event.organizer_positions.member_access.where(user:).exists? || event.self_and_ancestor_organizer_positions.manager_access.where(user:).exists?
    end

    if role.to_s == "manager"
      return event.self_and_ancestor_organizer_positions.manager_access.where(user:).exists?
    end
