...
class StripeAuthorization < ApplicationRecord
  ...
  def sync_from_stripe!
    ...
    if stripe_obj[:transact_so_ns].any?
      net_amount = -stripe_obj[:transact_so_ns].pluck(:amount).sum # must be negated since the rest of stripe_authorizations is treating positives as negatives in the interface
