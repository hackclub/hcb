      ...
      when_expanded do
        expose :amount_cents, documentation: { type: "integer" } do |hcb_code, options|
          ...
          if hcb_code.disbursement?
            # Card grants (source_subledger with no destination_subledger) always show positive
            if hcb_code.disbursement.source_subledger_id && hcb_code.disbursement.destination_subledger_id.nil?
              next hcb_code.disbursement.amount
            end
            next hcb_code.disbursement.signed_amount
          end
