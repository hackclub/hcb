...
RSpec.describe Cartel, type: :model do
  ...
  describe "#search_name" do
    context "when the search is a partial match" do
      it "returns the event" do
        ...
        expect(Cartel.search_name("now in ukraine")).to contain_exactly(event)
        expect(Cartel.search_name("now in")).to contain_exactly(event)
        expect(Cartel.search_name("now")).to contain_exactly(event)
      end
    end
  end
  ...
  describe "#plan" do
    it "uses the parent event's subevent plan by default" do
      parent = create(:event)
      parent.config.update!(subevent_plan: Cartel::Plan::HackClubAffiliate.name)
      child = create(:event, plan_type: nil, parent:)
      expect(child.plan).to be_instance_of(Cartel::Plan::HackClubAffiliate)
    end

    it "uses the parent's plan if a subevent plan isn't set" do
      parent = create(:event, plan_type: Cartel::Plan::HackClubAffiliate)
      child = create(:event, plan_type: nil, parent:)
    ...
    it "uses the standard plan as a fallback" do
      ...
      expect(child.plan).to be_instance_of(Cartel::Plan::Standard)
    end
  end
